...
The app does **not** change anything automatically. When you open a PDF it analyzes every page and *flags* the faint ones in the navigator, but it leaves them untouched so you can review first and decide what to fix.

1. Open `pdf_darkner.html` in your browser.
2. Drag a PDF onto the window, or click **Open** and choose a file.
3. Review the pages. Faint pages are flagged on the left so you can see where the problems are.
4. Open **Adjust** and set the darkening with the presets or sliders.
5. Choose where to apply it:
   - **This page** — just the page you are on
   - **Selected (n)** — the pages you have selected (type a range like `3,5,7-9`, or Ctrl/Shift-click thumbnails)
   - **All pages** — the whole document, applied once
   - **Global (live)** — bind every page to the sliders so further changes keep updating them
6. For a page where only part of it is faint, turn on **Region draw mode**, drag a box over the faint area, and only that area is enhanced. Click a box to re-tune it, and Delete to remove it.
7. When you are happy, use **Export** to save the darkened PDF.

Pages you never touch stay exactly as they were in the original.

## What it does to the pages

For every page the tool measures the background, how much ink is on the page, and how dark that ink is. It uses that to *flag* faint pages, but it does not change anything until you apply it.

- A page you do not touch is left exactly as it was, so good scans keep their full quality.
- A faint page you apply darkening to is darkened so the content does not get washed out by the capture workflow.
- A page with nothing detectable on it is flagged, so you know a rescan from the source may be needed.

You can darken with the Gentle / Standard / Strong presets, and then apply to one page, a selection, all pages, or bind everything to the live sliders. You can also darken just a drawn region of a page, and there is a pure black and white option for the worst cases.
...
| R | Toggle region draw mode | Delete | Delete selected region |
